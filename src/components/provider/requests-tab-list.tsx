...
import { ProviderRequestCard, type ProviderRequestCardData } from "./provider-request-card";
import type { RequestStatus } from "@/lib/constants";

const TABS: { key: string; label: string; statuses: RequestStatus[] | null }[] = [
  { key: "all", label: "Novas", statuses: ["PENDING"] },
  ...
  { key: "declined", label: "Recusadas", statuses: ["DECLINED"] },
  { key: "cancelled", label: "Canceladas", statuses: ["CANCELLED"] },
  { key: "everything", label: "Todas", statuses: null },
];

export function ProviderRequestsTabList({ requests }: { requests: ProviderRequestCardData[] }) {
    ...
    <div>
      ...
      {filtered.length > 0 ? (
        <div className="flex flex-col gap-3">
          {filtered.map((r) => (
            <ProviderRequestCard key={r.id} data={r} />
          ))}
        </div>
      ) : (
